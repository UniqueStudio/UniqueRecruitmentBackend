...
export const onMoveCandidate = (socket: Socket) => async (req: {
    cid: string;
    from: number;
    to: number;
    token: string;
    ...
    try {
        ...
        if (!candidate) {
            return socket.emit(
                'moveCandidateError',
                errorRes("Candidate doesn't exist!", 'warning', { cid, to, from })
            );
        }
        ...
        if (recruitment.end < Date.now()) {
            return socket.emit(
                'moveCandidateError',
                errorRes('This recruitment has already ended!', 'warning', { cid, to, from })
            );
        }
        if (step !== from) {
            return socket.emit(
                'moveCandidateError',
                errorRes('Candidate has been moved by others', 'warning', { cid, to, from: step })
            );
        }
